...
#script, fname = argv #labeling filename after the input agrument
...
#the elements.
def element_mass (*args):
    fname = './Isotope_properties_NIST.txt'
    '''
    ...
    while i < len(args):
        ...
        Elif is there for elements not isotopes, i.e "O" instead of "O16"
        The len(iso_mass_value[0] >=1) statement is to prevent O16 from entering
        that loop and only O to enter
        """
        f = open(fname)
        ## Read the first line
        line = f.readline()
            ...
            try:
                ...
                if isotope_name in line:
                    if iso_mass_value == iso:
                        mass = rx.findall(m) #separates the number and the estimate from NIST data
                        mass = float(mass[0]) #reports only the atomic mass
                        average_mass = rx.findall(avg)
                        average_mass = float(average_mass[0])
                        Element_mass.append(mass)
                        avg_element_mass.append(average_mass)
                        Isotope_label.append(isotope_name)
                        iso_mass_value_list.append(iso_mass_value)
                        print ("the mass of %s%s = %f " % (isotope_name, iso_mass_value, mass))
                    elif isotope_name == sym:
                        try:
                            if len(iso_mass_value[0]) >= 1:
                                pass
                        except IndexError:
                            mass = rx.findall(avg) #separates the number and the estimate
                            mass = float(mass[0]) #reports only the atomic mass
                            average_mass = rx.findall(avg)
                            average_mass = float(average_mass[0])
                            Element_mass.append(mass)
